...
@InitiatingFlow
@StartableByRPC
class PersonalVerifyFlow (private val counterParty: String, private  val linearId: UniqueIdentifier): FlowLogic<SignedTransaction>(){
...
private fun inputStateRef(): StateAndRef<PersonalState> {
    val criteria = QueryCriteria.LinearStateQueryCriteria(linearId = listOf(linearId))
    return serviceHub.vaultService.queryBy<PersonalState>(criteria).states.single()
}
private fun outputState():PersonalState{
    ...
    return PersonalState(input.fullName,input.age,input.birthDate,input.address,input.contactNumber,input.status,ourIdentity,counterRef,true)
}
    private fun userVerified(): TransactionBuilder{
        val counterRef = serviceHub.identityService.partiesFromName(counterParty, false).singleOrNull()
                ?: throw IllegalArgumentException("No match found for Owner $counterParty.")
        val notary = inputStateRef().state.notary
        val cmd = Command ( PersonalContract.Commands.Verify(), listOf(ourIdentity.owningKey, counterRef.owningKey))
...
@InitiatedBy(PersonalVerifyFlow::class)
class PersonalVerifyFlowResponder(val flowSession: FlowSession): FlowLogic<SignedTransaction>() {
